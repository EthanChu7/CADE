...
# modify
for i in range(0, 50):#pendulum
    for j in range(50, 100):#light
        if j == 100:
            continue
        ...
        if count != 4:
            ...
            theta_img = theta
        x = 10 + 8 * math.sin(theta_img)
        y = 10.5 - 8*math.cos(theta_img)
        # print(x, y)
        ...
        if count == 4:
          plt.savefig('./pendulum/test/a_' + str(int(i)) + '_' + str(int(j)) + '_' + str(int(shade)) + '_' + str(int(mid)) +'.png',dpi=96)
          count = 0
        else:
          plt.savefig('./pendulum/train/a_' + str(int(i)) + '_' + str(int(j)) + '_' + str(int(shade)) + '_' + str(int(mid)) +'.png',dpi=96)
        plt.clf()
        count += 1
