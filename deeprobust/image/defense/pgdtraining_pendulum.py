...
class PGDtraining(BaseDefense):
    ...
    def generate(self, train_loader, test_loader, label_idx=0, **kwargs):
        ...
        for epoch in range(1, self.epoch + 1):
            ...
            self.train(self.device, train_loader, optimizer, epoch, label_idx=label_idx)
            self.test(self.model, self.device, test_loader, label_idx=label_idx)
    ...
    def train(self, device, train_loader, optimizer, epoch, label_idx=0):
        """
        training process.
    ...
    def test(self, model, device, test_loader, label_idx=0):
        """
        testing process.
        ...
        for data, target in test_loader:
            ...
            data_adv, output_adv = self.adv_data(data, target, ep = self.epsilon, num_steps = self.num_steps, clip_max=self.clip_max, clip_min=self.clip_min)
